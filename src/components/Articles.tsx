...
const ReadArticleSection = () => {
  ...
  return (
    <section className="border-t border-[#007E85]/20 py-16 px-4">
      <div className="container mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12 items-start">
        {/* Left Side */}
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-[#007E85] mb-4">
          ...
          <Link to="/articles">
            <Button className="bg-[#007E85] hover:bg-[#006A6F] text-white px-6 py-3 rounded-md">
              See all articles
            </Button>
          </Link>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {articles.map((article) => (
            <Link
              ...
              key={article.id}
              className="group rounded-lg shadow-sm hover:shadow-md border border-[#007E85]/20 overflow-hidden transition"
            >
              <img
                src={article.image}
                alt={article.title}
              ...
              <div className="p-4 space-y-2">
                <p className="text-xs font-semibold text-[#007E85] uppercase tracking-wide">
                  {article.category}
                </p>
